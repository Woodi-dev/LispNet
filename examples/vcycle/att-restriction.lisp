...
(defmethod initialize-instance :after ((layer att-restriction-layer) &rest initargs)	  
  (if (> (filters layer) 1) 
      (progn
	(setf (restrict-conv layer) (create-layer 'conv2d-layer  (model layer) :in-channels (filters layer) :out-channels 1 :kernel-size 3 :padding "same" :strides '(2 2) :kernel-initializer (kernel-initializer layer)  :trainable (trainable layer) :use-bias nil))
	(setf (att-conv layer) (create-layer 'conv2d-layer (model layer) :in-channels 1 :out-channels (filters layer) :kernel-size 3 :padding "same" :activation #'channel-softmax :trainable (trainable layer) :use-bias nil)) ;;
